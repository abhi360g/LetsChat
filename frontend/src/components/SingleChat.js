...
const SingleChat = ({ fetchAgain, setFetchAgain }) => {
  ...
  const sendMessage = async (event) => {
    if (event.key === "Enter" && newMessage) {
      ...
      try {
        const config = {
          headers: {
            "Content-Type": "application/json", // Because we are sending json data
            Authorization: `Bearer ${user.token}`,
          },
        };

        setNewMessage(""); // Since this function is asyn setNewMessage won't make the newMessage as "" instantly
  ...
  const typingHandler = (e) => {
    ...
    if (!typing) {
      setTyping(true);
      socket.emit("typing", selectedChat._id); //Send it to the selectedChat id room
    }
      ...
      {selectedChat ? (
        ...
        >
          <Text fontSize="3xl" pb={3} fontFamily="Work sans">
            Click on a user to start chattinggggggg
          </Text>
        </Box>
      )}
    </>
